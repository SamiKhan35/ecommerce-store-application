...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    DecQuantity(state, action) {
      ...
      if (state.productsCart[productIndex].quantity >= 1) {
        state.productsCart[productIndex].quantity -= 1;
        state.totalQuantity -= 1;
      } else if (state.productsCart[productIndex].quantity < 0) {
        const deleteItems = state.productsCart.filter(
          (item) => item.id !== action.payload.id
        );
        state.productsCart = deleteItems;
      }
      localStorage.setItem("cartItems", JSON.stringify(state.productsCart));
